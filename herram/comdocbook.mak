...
# CATALOG_DSSSL Directory with stylesheets DSSSL for DocBook
...
dbrep_html_jade: $(INDEX) $(SOURCES) $(IMAGES) $(XSLT_HTML) $(PROYECTO)-4.1.2.$(EXT_DOCBOOK)
	$(MKDIR) -p $(HTML_DIR)
	for i in $(IMAGES) ; do $(CP) $$i $(HTML_DIR)/`basename $$i`; done 
	-bp=`pwd`;cd $(HTML_DIR) && $(RM) -f *.aux *.log && SP_ENCODING=UTF-8 $(JADE) -c$(CATALOG_DSSSL) -V html-backend -D$(DOCBOOK_DSSSL)/html -t sgml -ihtml -d $$bp/$(DSSSL_HTML) $(SGML_XML) $$bp/$(PROYECTO)-4.1.2.$(EXT_DOCBOOK)

dbrep_html_jade_single: $(INDEX) $(SOURCES) $(IMAGES) $(XSLT_HTML) $(PROYECTO)-4.1.2.$(EXT_DOCBOOK)
	$(MKDIR) -p $(HTML_DIR)
	for i in $(IMAGES) ; do $(CP) $$i $(HTML_DIR)/`basename $$i`; done 
	-bp=`pwd`;cd $(HTML_DIR) && $(RM) -f *.aux *.log && SP_ENCODING=UTF-8 $(JADE) -c$(CATALOG_DSSSL) -V nochunks -V html-backend -D$(DOCBOOK_DSSSL)/html -t sgml -ihtml -d $$bp/$(DSSSL_HTML) $(SGML_XML) $$bp/$(PROYECTO)-4.1.2.$(EXT_DOCBOOK) > $(PROYECTO).html
...
dbrep_ps_jade: $(PRINT_DIR)/$(PROYECTO).dvi
	cd $(PRINT_DIR) && $(DVIPS) -t $(PAPEL) -o $(PROYECTO).ps $(PROYECTO).dvi
...
$(PRINT_DIR)/$(PROYECTO).tex: $(INDEX) $(SOURCES) $(IMAGES:.png=.eps)  $(PROYECTO)-4.1.2.$(EXT_DOCBOOK)
	...
	-bp=`pwd`; cd $(PRINT_DIR) && $(RM) -f *.aux *.log && SP_ENCODING=UTF-8 $(JADE) -V tex-backend -c$(CATALOG_DSSSL) -D$(DOCBOOK_DSSSL)/print -o $(PROYECTO).tex -t tex -d  $$bp/$(DSSSL_PRINT) $(SGML_XML) $$bp/$(PROYECTO)-4.1.2.$(EXT_DOCBOOK)
