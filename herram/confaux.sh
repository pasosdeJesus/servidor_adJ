...
function addOcamlConfv  {
	...
	tifs=$IFS;
	IFS="%";
	LC_ALL=es_CO.UTF-8 LANG=es_CO.UTF-8 LANGUAGE=es_CO.UTF-8 grep -A 1 "^[A-Za-z_0-9]*[ \t]*=.*"< $confvar_file  | \
	tr "\n" "#"  | \
	sed -e "s/--#//g"  |  \
	sed -e 's|[ \t]*\([A-Za-z_0-9]*\)[ \t]*=[ \t]*["]\([^"]*\)["][^#]*## \([^#]*\)#|\1%\2%\3#|g'  | \
	tr "#" "\n" |
	while read nvar vval comm ; do  
		ovar=`echo $nvar | tr A-Z a-z`; 
		oval=`echo $vval | sed -e 's|\$\([A-Za-z_0-9]*\)|"^\1^"|g'` ; 
		echo "let $ovar=\"$oval\";;" >> confv.tm1 ; 
		echo "(** $comm *)" >> confv.tm1; 
		echo "" >> confv.tm1 ; 
		echo "s|\"\^$nvar\^\"|\"\^$ovar\^\"|g" >> confv.tm2 ; 
	done 
